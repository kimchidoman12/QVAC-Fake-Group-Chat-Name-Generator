#!/usr/bin/env node
// QVAC Fake Group Chat Name Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29423;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/groupchatname") {
      try {
        ...
        const result = await generate(modelId, group.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Group Chat Name Generator GUI ready at http://localhost:${PORT}`);
  });
